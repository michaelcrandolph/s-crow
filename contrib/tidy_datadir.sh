...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete s-CROW database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no s-CROW datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old s-CROW datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected s-CROW 0.7 datadir."
    ;;
  3)
    echo "Detected s-CROW pre-0.8 datadir."
    ;;
  4)
    echo "Detected s-CROW 0.8 datadir."
    ;;
esac
