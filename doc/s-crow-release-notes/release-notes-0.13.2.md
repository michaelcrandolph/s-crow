s-CROW Core version 0.13.2 is now available from:

  <https://download.s-crow.org/s-crow-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/s-crow-project/s-crow/issues>
...
updates will be released anymore. Without security updates, using a s-crow
...
ECDSA signatures inside s-CROW transactions now use validation using
...
Previous versions of s-CROW Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of s-CROW Core was to raise the default minimum
relay fee.

s-CROW Core 0.13.2 will have a strict maximum size on the mempool. The
...
s-CROW Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running s-crowd without having to do any manual configuration.
...
s-CROW Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. s-CROW Core will relay transactions with
...
entirely in s-CROW Core 0.13, and it is at this time undecided whether the
...
s-CROW Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
s-CROW Core automatically creates a hidden service to listen on, without
manual configuration. s-CROW Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if s-CROW Core is listening, and
...
s-CROWd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals s-CROW
Core to use floating fees. By default, s-CROW Core will use floating
fees.
...
(default: `0.0002` SCW/kB).

At all times, s-CROW Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SCW.
Furthermore, s-CROW Core will never create transactions paying less than
...
`s-crow.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 29711 to a RPC server bound on localhost on port 49712 do:

    stunnel -d 29711 -r 127.0.0.1:49711 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /s-crowrpc>
        ProxyPass http://127.0.0.1:9711/
        ProxyPassReverse http://127.0.0.1:9711/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass s-crowd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< s-crowrpc:<password>
    </Location>
...
Restarting s-crowd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in s-crow.conf
...
For example:
    ...
    $ src/s-crow-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The s-CROW Core code base has
...
- `s-crow-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `s-crow-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the s-CROW scripting system that in combination with
...
s-CROW transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of s-CROW Core.
...
The two phases are distinct in the s-CROW-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming s-CROW wallet
  software more difficult and which seriously complicates the design of smart
  contracts for s-CROW.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each s-CROW block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of s-crows they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of s-crows being spent is exactly
...
- **Increased security for multisig:** s-CROW addresses (both P2PKH addresses
  ...
  believed to be provided by s-CROW's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of s-CROW's current ledger (called the UTXO set),
  ...
  note that s-CROW Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  s-CROW users to individually opt-in to almost any change in the s-CROW
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and s-CROW Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the s-CROW
client to s-CROW Core 0.13.2 to signal for segwit support. When a
...
Since s-CROW Core 0.10.0, nodes have defaulted to only relaying and
...
- `s-crow-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `s-crow-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
